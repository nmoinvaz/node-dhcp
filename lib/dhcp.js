...
function Server(config, listenOnly) {
  ...
  sock.on('message', function(buf, rinfo) {
    ...
    self.emit('message', req, rinfo);

    if (!listenOnly) {
      // Handle request
      switch (req.options[53]) {
        case DHCPDISCOVER: // 1.
          self.handleDiscover(req, rinfo);
          break;
        case DHCPREQUEST: // 3.
          self.handleRequest(req, rinfo);
          break;
        case DHCPINFORM: // 8.
          self.handleInform(req, rinfo);
          break;
        default:
...
Server.prototype = {
  ...
  handleDiscover: function(req, rinfo) {
    ...
    this.sendOffer(req, rinfo);
  },
  sendOffer: function(req, rinfo) {
    ...
    this._send(this.config('broadcast'), ans, rinfo);

  },

  handleRequest: function(req, rinfo) {
    ...
    this.sendAck(req, rinfo);
  },
  handleInform: function(req, rinfo) {
    ...
    // Send the actual data
    // INADDR_BROADCAST : 68 <- SERVER_IP : 67
    this._send(INADDR_BROADCAST, ans, rinfo);
  },
  sendAck: function(req, rinfo) {
  ...
  _send: function(host, data, rinfo) {
    const sb = Protocol.format(data);

    this._sock.send(sb._data, 0, sb._w, rinfo.port || CLIENT_PORT, host, function(err, bytes) {
      if (err) {
        console.log(err);
      } else {
